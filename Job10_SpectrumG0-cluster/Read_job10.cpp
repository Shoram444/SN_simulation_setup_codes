...
void Read_job10()
{
    const char* DIRPATH = "/sps/nemo/scratch/mpetro/Projects/PhD/Cluster/Data/Job10_10f_1e5events_final/";
	...
	for (int fld = 0; fld < 100; fld++)
	{
		...
		{
	    	cout << "Default.root DOESNT EXIST - PATH: " << fpath.str().c_str() << endl;
	    } 
	    else 
	    {
	    	cout << "READING FILE : " << fld << endl;
	    	cout << fpath.str().c_str() << endl;

	    	TFile* 	   inFile 	= new TFile(fpath.str().c_str());
