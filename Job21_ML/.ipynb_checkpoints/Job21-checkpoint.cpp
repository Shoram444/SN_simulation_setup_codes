	...
	/////////////////////////////////////////////////////////////
			...
			{ 	
				...
				if(
					...
					// miFilter->getTwoNegCh() &&
					miFilter->getTwoPart() 
					// miFilter->getAll()   
					) // only particles that have passed 2vbb filters, excluding 2 negatively charged particles since B is off
				{
					...
					float_t  x1AtOM  = eve->getSD()->getvisuhitv()->at(lastOuttaGas1).getStart()->getX();  
					float_t  y1AtOM  = eve->getSD()->getvisuhitv()->at(lastOuttaGas1).getStart()->getY();  
					float_t  z1AtOM  = eve->getSD()->getvisuhitv()->at(lastOuttaGas1).getStart()->getZ();

					float_t  x2AtOM  = eve->getSD()->getvisuhitv()->at(lastOuttaGas2).getStart()->getX();  
					float_t  y2AtOM  = eve->getSD()->getvisuhitv()->at(lastOuttaGas2).getStart()->getY();  
					float_t  z2AtOM  = eve->getSD()->getvisuhitv()->at(lastOuttaGas2).getStart()->getZ();


					TVector3* r1Escaped = new TVector3(x1Escaped, y1Escaped, z1Escaped);  	// position vector of the foil vertex
					TVector3* r2Escaped = new TVector3(x2Escaped, y2Escaped, z2Escaped);
					TVector3* r1AtOM = new TVector3(x1AtOM, y1AtOM, z1AtOM);     			// position vector where electron hits OM. Tracklength is calculated as sqrt(r1Escaped^2 + r1AtOM^2)
					TVector3* r2AtOM = new TVector3(x2AtOM, y2AtOM, z2AtOM);
