...
#SBATCH --job-name=combP         	 # Job name
...
for sim_folder in $MAIN_DIR/Job23*; do
    ...
    if [ -d "$sim_folder" ]; then
        ...
        if [ -e "$sim_folder/0/EnePhiDist_Job23_SDBDRC_probabilityCut_100mm_prob.root" ]; then
            echo "$sim_folder/0/EnePhiDist_Job23_SDBDRC_probabilityCut_100mm_prob.root exists!"
            
            /sps/nemo/sw/BxCppDev/opt/root-6.16.00/bin/hadd -f $sim_folder/combined_vertex_probability_cut.root $sim_folder/*/EnePhiDist_Job23_SDBDRC_probabilityCut_100mm_prob.root
        fi
    fi
done
