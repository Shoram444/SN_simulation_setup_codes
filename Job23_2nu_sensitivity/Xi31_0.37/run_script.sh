...
cp $TMPDIR/CDCut.brio %MAIN_FOLDER/%USER_FOLDNAME/%k/%i/CDCut.brio              # if you want to keep CD.brio
...
%FAL/flreconstruct -i CDCut.brio -p %CONF_FOLDER/SNCutsPipeline_vertex.conf -o CDCut_vertex.brio
%FAL/flreconstruct -p %CONF_FOLDER/p_MiModule_v00.conf -i CDCut_vertex.brio 

cp $TMPDIR/CDCut.brio %MAIN_FOLDER/%USER_FOLDNAME/%k/%i/CDCut_vertex.brio              # if you want to keep CD.brio

ls -l

echo "FINISHED vertex cut flreconstruct!"
...
%ROOT_FOLDER/root -l -b Job23.cpp
mv EnePhiDist_Job23.root %MAIN_FOLDER/%USER_FOLDNAME/%k/%i/EnePhiDist_Job23_SDBDRC_vertexCut_100mm.root
